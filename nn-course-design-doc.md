...
- Not a full port of Nielsen's book. Chapters 5 and 6 are covered conceptually, not implemented (see Chapter 8).
...
- Prose policy: write original explanations structured for interactivity. Verbatim borrowing is legally fine with attribution, but keep it rare and deliberate (only where Nielsen's phrasing is clearly the best version). Each chapter links to the corresponding book chapter as the optional "go deeper" path.
...
- **State:** localStorage for chapter progress, saved editor code per exercise, and completion flags. *(Superseded: one document, plus a derived per-exercise projection so older builds and older exports still work.)* "Reset module" and "Reset all" controls. Export/import progress as JSON (nice-to-have).
...
### Exercise/test contract (uniform across chapters)
...
- Gating: a chapter's "next" unlock requires its tests passing, but free navigation to any previously seen content is always allowed.

The gradient checker (Chapter 5) is the flagship test: compares analytic gradients from the learner's backprop against central-difference numerical gradients on a tiny fixed network, elementwise, with tolerance 1e-7 relative. This is the single strongest correctness guarantee in the course and should be prominently celebrated in the UI when it passes.

## 4. Chapters

Conventions per chapter: opens with "What you'll be able to do after this" (2-3 items), body interleaves short prose beats (150-400 words each) with interactives and exercises, closes with a recap and a "Go deeper" link to the corresponding Nielsen chapter.

### Chapter 1: From neurons to networks
...
- **Exercise:** Implement `sgd_step` using a provided **numerical** gradient function (finite differences), and a mini-batch loop. Train a tiny network on a toy 2D dataset live. It works but is visibly slow; the chapter ends by quantifying why (one gradient estimate costs one forward pass per parameter) to set up backprop.
- **Book link:** Chapter 1 (learning section).

### Chapter 4: Backpropagation, the idea
...
### Chapter 5: Backpropagation, for real
- **Covers:** Implementing BP1-BP4 in NumPy.
- **Exercise (the summit):** Implement `backprop(weights, biases, x, y)` returning gradient lists. Test ladder: (1) shape checks, (2) fixed-fixture value checks on a tiny net, (3) the numerical gradient check, (4) integration: plug into the Chapter 3 SGD loop, train 784-30-10 on the MNIST subset, live loss/accuracy chart, target >= 88% test accuracy.
- **UI:** Prominent success state when the gradient check passes. Show a wall-clock comparison against Chapter 3's numerical-gradient training to make the speedup visceral.
- **Book link:** Chapter 2.

### Chapter 6 (Interlude): Universality
...
- **Book link:** Chapter 4. (Note the reorder: Nielsen's Chapter 3 maps to our Chapter 7.)

### Chapter 7: Making it actually work
...
- **Structure:** Three short cycles, each: problem demo -> small code change to the learner's own network from Chapter 5 -> before/after training comparison chart. Overfitting demo uses a deliberately small training slice (1,000 images) so the train/test gap is dramatic.
- **Exercises:** `cross_entropy_delta`, `l2_update` (modify the weight update rule), `smart_init`. Each is a focused diff, not a rewrite.
- **Book link:** Chapter 3.

### Chapter 8: Why deep is hard (and what came next)
...
- **Covers conceptually, does not implement:** convnets (Nielsen Ch. 6). From-scratch convolutions in Pyodide are slow and add little conceptual return; say this openly in the chapter and link the chapter.
...
- Every chapter's notation must match the global shape conventions exactly. Notation drift between chapters is a bug.
...
/src/                React app
/src/modules/NN/     one folder per chapter: content.mdx (or .tsx), interactives/
...
- **M1, exercise pipeline:** Editor -> worker -> tests -> results UI, with the three-stage hint reveal and localStorage persistence. Build using Chapter 2's exercise as the guinea pig.
- **M2, spine:** Chapters 1-3 complete (content + interactives + exercises).
- **M3, summit:** Chapters 4-5, including the step-through visualization and gradient checker. This is the highest-effort, highest-value milestone.
- **M4, tail:** Chapters 6-8.
...
- MDX vs. TSX for chapter content authoring (decide in M1 based on friction). **TSX.** Prose beats are short and always interleaved with components, so MDX would have added a dependency without saving friction.
- Whether Chapter 5's big training run should offer a "use reference backprop" fallback so a stuck learner can still experience Chapters 7-8 (recommended: yes, clearly labeled). **Yes, and it fell out of the architecture rather than being built.** Chapters 7 and 8's panels take their gradient from `course.backprop`, so no panel after Chapter 5 requires the learner's own version; only Chapter 5's own payoff run does. Chapter 5's locked panel and the start page both say so.
...
- **Chapter 8 grew a payoff run and a second engine.** The design doc called for per-layer gradient bars with a depth slider and a sigmoid/ReLU toggle; that is `LayerSpeedBars` (TypeScript, so the bars move as fast as the slider does), and beside it `DepthTrainPanel` trains one hidden layer against four with the learner's own code in Pyodide. Two engines means two sets of numbers from two random generators: `tools/bench_layer_speeds.ts` and `tools/bench_depth.py` regenerate them, and CLAUDE.md records that a number from one must never be quoted for the other.
- **The optional ReLU exercise was not built.** Chapter 8 swaps the squash inside its own panel instead, which gets the comparison without asking the learner to rewrite `backprop` after the course has already taken its summit.
- **A tenth chapter was added, and with it a fourth goal.** The design doc's
  non-goals said "not a production ML tool", and nothing in Chapters 1 to 9 shows a
  learner how to point the network at a problem of their own: every dataset in the
  course arrives scaled, numeric, complete, labelled and split. Chapter 10 is that
...
- **A ninth chapter was added** (not in the design doc), after a teaching review of
  the finished course found that the learner owned seven functions but had never
  assembled them: every training run was started by a course-written panel. Chapter 9
  is the loop. The review also added the validation split to Chapter 7, which had been
...
- **The vocabulary handover was distributed, and the reading list moved to the end**
  (later still, on the primary learner's report that he skimmed Chapters 9 and 10 and
  that the field's terms arrived as one dump to swap in his head). Chapter 9's
  twenty-row translation table became a naming note in each of Chapters 1 to 8, at the
  first use of the thing named, and the where-to-go-next list moved from Chapter 9 to
  Chapter 10, which is the last page. Chapter 9 had been closing with a reading list one
...
- **A start page was added** (not in the design doc): what the course is, how the machinery works, every chapter, the training demo that used to sit on a tab of its own, and the stored progress with save, load and reset. It is where a bare link lands.
- **Gating stayed softer than section 3 proposed.** Nothing locks a chapter; what an exercise gates is the panel that runs the learner's code, since there is nothing to run until the code exists.
- **Three of section 4's names and numbers were never what shipped**, and are recorded here rather than edited above. Chapter 7's exercises are `l2_step` and `init_network`, not `l2_update` and `smart_init`: both were renamed for what they do to the learner's own network rather than to the update rule in the abstract, and `course_helpers.py` carries the shipped names. Chapter 8's depth slider runs 1 to 5 hidden layers rather than 2 to 6, because one hidden layer is the network the learner already owns from Chapter 5 and is the only honest baseline for the comparison. And the pretrained weights ship as `pretrained_weights.json.gz` rather than an npz, for the reason CLAUDE.md records: Chapter 2's diagram reads the same file in JS, which has no npz parser.
- **The visual identity became a shared series layer, after the course was finished.** Section 3 said nothing about design beyond the stack, and the course shipped looking like an unstyled document with a green accent. `src/brand/` now holds a nine-hue accent family computed at one OKLCH lightness and chroma, four named type roles, and the masthead, tab strip, link and footer chrome, so a second course on another topic reads as a sibling rather than as an unrelated page. `course-kit/BRAND.md` is its spec. (The kit later moved to the series repository, `PavolC/moving-parts`, so that path is now there rather than here.)
...
- **The nine editors became one file in one panel**, after the primary learner
  ...
  on each chapter." Section 3 above says "one editor pane per exercise, pre-filled
  ...
  see it.
- **The method was extracted into `course-kit/`.** Not a departure from the design so much as a second deliverable the design did not anticipate: this repo's `CLAUDE.md` grew from 629 words to 5,000 across five days, almost entirely out of one learner's reported confusions, and most of it is not about neural networks. The kit is that file with the topic removed, plus the process that generated it, the fifteen incidents behind its rules, this template's own conventions, the brand, and six slash commands. (It lived in this repository while it was still being refined by building this course; with a second course starting it moved up to the series repository, `PavolC/moving-parts`, and this repo no longer carries a copy.)
