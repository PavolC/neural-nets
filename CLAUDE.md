...
- **The series mark and the course mark have separate jobs.** `Monogram.tsx` draws the
  ...
  descriptors and taglines sit outside that pair, and the footer's description starts on
  the line below it. `theme-color` is also a literal because browser chrome needs it
  before JavaScript runs. `python3 tools/check_brand.py` checks both marks and the
  literals it can reach from this repository.
