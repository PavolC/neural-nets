...
const VERDICTS = {
  ...
  "two fail, four pass, two names borrowed": {
    ...
    tests: [
      ...pass.slice(0, 4),
      { name: "tf1", title: "fire computes sigmoid(w . x + b)", passed: false, section: null,
        message: "expected 0.7311 for w=[1,1], b=0, x=[1,0], got 0.5. Check that the bias is added before the squash." },
      { name: "tf2", title: "the bias shifts the neuron's output", passed: false, section: null,
        message: "raising b by 1 should raise the output; it fell." },
    ],
  },
  "the file does not run": {
    setup_error: { message: "SyntaxError: invalid syntax", line: 34, section: "sigmoid-neuron" },
    passed: false, lent: [], tests: [],
  },
  "the failure is upstream": {
    setup_error: null, passed: false, lent: [],
    tests: [{ name: "tx", title: "feedforward's two-layer values", passed: false, section: "sigmoid-neuron",
...
for (const [label, verdict] of Object.entries(VERDICTS)) {
  ...
  await ctx.route("**/pyodide/**/pyodide.mjs", (route) =>
    route.fulfill({ status: 200, contentType: "text/javascript", body: fakeModule(verdict) }));
...
// The other two ways to start a run, which are the notebook affordances.
{
  ...
  await ctx.route("**/pyodide/**/pyodide.mjs", (route) =>
    route.fulfill({ status: 200, contentType: "text/javascript",
                    body: fakeModule(VERDICTS["everything passes"]) }));
  const p = await ctx.newPage();
...
console.log("\nthe run path holds: every verdict renders, and all three ways to start a run work.");
