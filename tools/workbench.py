...
def body(section_id, kind):
    ...
    kind is "solution", "skeleton" or "seam". Given sections have exactly one
    body and ignore the kind, because there is nothing for the learner to
    write in them.
    """
    s = BY_ID[section_id]
    if s["kind"] == "given":
        name = "cost" if section_id == "given-cost" else "batch"
...
def closure(section_id, seen=None):
    """section_id and everything it needs, transitively, in table order."""
