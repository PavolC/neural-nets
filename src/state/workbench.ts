...
// the splices). This file owns the stored copy: how it is created out of nine
// older per-exercise documents, how a section is seeded the first time the
...
/** Keys under gn:v1: that a progress file must not carry. The undo buffer is
...
export function sectionDefines(id: string): boolean {
  ...
  const def = SECTION_BY_ID.get(id);
  if (!def) return false;
  return def.provides.every((name) =>
    new RegExp(`^\\s*def\\s+${name}\\s*\\(`, "m").test(body),
  );
}
...
export function editedGivens(): SectionDef[] {
  return SECTIONS.filter(
    (s) => s.kind === "given" && sectionPresent(s.id) && !isUntouched(s.id),
  );
}
...
export function putSection(id: string, body: string): SpliceResult {
  ...
  const problem = doc.problems.find(
    (p) => p.kind !== "out-of-order" && (p.id === undefined || p.id === id),
  );
  if (problem) {
    return {
      ok: false,
