...
export function Module9() {
  return (
    <article className="module">
      <h2>Chapter 9: Assembling the program</h2>
      <AfterThis
        items={[
      ...
      <p>
        ...
        watched since Chapter 5 was started by a panel, and the panel did the
        loading, the epoch loop and the scoring around your code. This chapter is
        that loop, and it is one function long.
      </p>

      <SectionHeader id="c9-loop" title="The loop that was never yours" />
      <p>
        Count what those fifteen give you: a squash and one neuron (Chapter 1), a
        whole network's forward pass as one matrix multiplication per layer
        (Chapter 2), a descent step and the loop around it (Chapter 3), the four
        equations that turn one forward pass into every parameter's partial
        derivative (Chapter 5), and a cost, a starting draw and an update rule
        (Chapter 7). Every training run in this course has been made of those
      ...
      <p>
        Chapter 3's <code>sgd</code> came closest: the right loop, shuffle, cut,
        step, repeat, running on your own gradient in every panel that has trained
        it since Chapter 5. But the course only ever called it one epoch at a time,
        and it has never drawn the network it trains, never scored one, and never
        taken a decayed step. In your file, <code>sgd</code> still calls Chapter 3's
      ...
      <p>
        ...
        because every accuracy figure in eight chapters was computed for you, but
        every ingredient is from Chapter 2. <code>feedforward</code> on a batch
...
(guesses == y).mean()             ->  0.6666...          two of three`}</pre>
      ...
      <SectionHeader id="c9-run" title="Your program, on the digits" />
      <p>
        ...
        mini-batch adapter written for you in Chapter 5, every line in the map below
        is code you wrote. The panel runs your file and nothing is substituted
        underneath it, which is what nine chapters of writing code into one file
        were for.
      </p>
      <Figure caption="Your program's call map, indented by who calls whom, each line labeled with who wrote it and where.">
        <div className="table-scroll scroll-x" tabIndex={0}>
          <pre className="torch-listing">{`train(sizes, X, Y, X_test, y_test, ...)   yours, this chapter
    init_network(sizes, rng)              yours, Chapter 7
    each epoch: rng.permutation(n)        NumPy's shuffle
        each mini-batch:
            batch_gradient(...)           written for you, Chapter 5
                backprop(...)             yours, Chapter 5
                    cross_entropy_delta   yours, Chapter 7
            l2_step(...)                  yours, Chapter 7
        accuracy(w, b, X_test, y_test)    yours, this chapter
            feedforward(w, b, X)          yours, Chapter 2`}</pre>
        </div>
      </Figure>
      <p>
      ...
      <p>
        ...
        epochs, while Chapter 7's run of the same network reported 92.1. Did
        assembling the program yourself cost you almost two points? No: the decay is
        switched on here (lambda 1, which Chapter 7's run did not use), and one
        ...
        different stream through it. Chapter 7 measured what a different stream alone
      ...
      <SectionHeader id="c9-step-size" title="The step size, from the inside" />
      <p>
        Only one of the other two settings reaches past Chapter 7's grid: 3.0
        ...
        That is the overshoot Chapter 3 drew on a bowl, at the scale of a real
        network.
      </p>
      <Aside>
        <p>
          ...
          Chapter 7 said what to do about it, and this is the moment the temptation is
      ...
      <SectionHeader id="c9-program" title="A file that runs anywhere" />
      <p>
      ...
      <Figure caption="Your downloaded file, driven from outside this page. The import needs NumPy installed; X, Y, X_test and y_test are whatever data you put in the packing Chapter 2 fixed, and Chapter 10 is about producing them from a file nobody prepared.">
        <div className="table-scroll scroll-x" tabIndex={0}>
...
0.904`}</pre>
      ...
      <SectionHeader id="c9-built" title="What you built" />
      <p>
        In Chapter 1 a neuron was a picture on a page. Here is what is running in
        your browser now, all of it written by you:
      </p>
      <ul className="m9-built-list">
        <li>
          <b>sigmoid and one neuron</b> (Chapter 1), the squash and the
          weighted-sum-plus-bias every layer since has been made of.
        </li>
        <li>
          <b>feedforward</b> (Chapter 2), a whole network as one matrix
          multiplication per layer, running 784-30-10 on real digits.
        </li>
        <li>
          <b>sgd_step and sgd</b> (Chapter 3), gradient descent with mini-batches
          and a shuffle.
        </li>
        <li>
          <b>backprop</b> (Chapter 5), BP1 to BP4 in about fifteen lines, checked
          entry by entry against numerically measured gradients, and training
          784-30-10 to 89 percent in seconds where Chapter 5's own timing put the
          nudge-and-measure route at hours.
        </li>
        <li>
          <b>cross_entropy_delta, init_network and l2_step</b> (Chapter 7), three
          one-line changes worth about three points of accuracy and a great deal of
          stability.
        </li>
        <li>
          <b>train and accuracy</b> (this chapter), the loop that runs all of the
      ...
      <SectionHeader id="c9-words" title="Which words were only ours" />
      <p>
        Each chapter named the field's word for its own ideas as it went. The squash
        is an activation function and the line a neuron cuts is its decision
        boundary (Chapter 1); a knob is a parameter, the cost is the loss, and one
        knob's slope is a partial derivative (Chapter 3); a factor is a local
        derivative and blame is the error (Chapter 4); receipts are the cached
        forward pass (Chapter 5); the divided start is Xavier initialization and the
        decay factor is L2 regularization (Chapter 7); a layer's learning speed is
        its gradient norm, and a hop well under 1 is the vanishing gradient problem
        (Chapter 8). The whole lookup, both ways round, is folded into the start page
        ...
        <table className="truth-table">
          ...
          <tbody>
            <tr>
              ...
              <td>
                one dataset carried through nine chapters, so that a new idea never
      ...
      <SectionHeader id="c9-missing" title="The two jobs you did by hand" />
      <p>
      ...
      <p>
        ...
        automates is exactly what you did in Chapter 5: your <code>zs</code> and{" "}
        <code>activations</code> lists are a hand-built version of that record, and
        Chapter 5 called them the cached forward pass for this reason. This is why a
      ...
      <p>
        ...
        direction builds speed and Chapter 3's zigzag across a valley cancels itself
      ...
      <p>
        ...
        in Chapter 5's panel. Looking once is a demonstration rather than a habit.
      </p>
      <p>
        Chapter 10 points the program you just assembled at a file with none of the
        ...
        Chapter 7's argument for dividing the weights by <M tex="\sqrt{n}" /> counted
