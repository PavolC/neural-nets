...
def _crash_message(err, section, target_id=None):
    ...
    if section.get("kind") == "given" and "backprop()" in err["message"]:
        # The one predictable arity failure: batch_gradient calls backprop
        # with a swapped-in BP1, and the learner has not made room for it.
        # Blaming the written-for-you section blames code they cannot fix.
        return (
