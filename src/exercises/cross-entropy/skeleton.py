"""Exercise: the cross-entropy cost.

Two small functions. Neither one is an algorithm: the first is a formula
over a batch, the second is a subtraction.

Provided by the course:
- feedforward(weights, biases, X): from Module 2. Works on one column or a
  whole batch of columns.
- sigmoid_prime(z), backprop(weights, biases, x, y, output_delta): your
  Module 5 work, handed back. backprop's last argument is the function that
  supplies BP1's output-layer blame.
