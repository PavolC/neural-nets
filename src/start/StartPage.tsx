...
import { downloadText, hasBackup, restoreBackup, sectionState, SECTIONS } from "../state/workbench";
...
export function StartPage({ onGoTo }: { onGoTo: (moduleId: string) => void }) {
	...
	const restore = () => {
		if (!window.confirm("Put back the nine separate documents you had before the exercises became one file, and rebuild the file from them? Anything written since then is lost.")) return;
		const ok = restoreBackup();
		say(ok ? "Restored. Your file was rebuilt from the nine documents." : "There is nothing to restore in this browser.", !ok);
	};
	...
	const load = (file: File) => {
		file.text()
			.then((text) => {
				...
				// a file merged into a workbench that already existed is the one case
				// where something of this browser's was replaced.
				say(
					report.shape === "merged"
	...
	return (
		<article className="module start-page">
			...
			<p>
				It assumes you can read Python and have seen high-school algebra: what a graph is, what a slope is, and how to rearrange <M tex="y = mx + c" />. That is the floor, and nothing below it
				is assumed.
			</p>
			<p>
			...
			<ul className="start-facts">
				...
				<li>
					<b>Each exercise shows its work.</b> The tests are readable in the page, hints come in three stages that you choose to open, and the reference solution is one of them. When a run
          ...
          cannot claim a number the list does not contain. */}
			...
			<p>
				Everything you write in this course goes into one Python file, a section per exercise, in the order you meet them. {passed.length} of {EXERCISES.length} sections are passing here.{" "}
				...
				{SECTIONS.map((section) => {
					...
					return (
                      ...
                      arrived, not what it is. */}
									{section.kind === "given" ? (state === "missing" ? "not in your file yet" : "in your file") : STATE_WORDS[state]}
								</span>
			...
			<p className="start-storage-note">
				A saved file holds your whole nn.py, which hints you opened, and which sections passed. Loading one replaces the sections it names and leaves the rest alone. Downloading nn.py is a
				different thing: that is the Python itself, ready to run anywhere NumPy is installed, and it is not a file this page can load back.
			</p>
		</article>
	);
}
