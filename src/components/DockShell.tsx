...
export function DockShell({
  ...
  const resolveWidth = useCallback((stored: number) => {
  ...
  const columnWidth = window.innerWidth - applied;
    ...
    >
      ...
      {open && (
        <div
          ...
          aria-valuemin={dockBounds(window.innerWidth).min}
          aria-valuemax={dockBounds(window.innerWidth).max}
          tabIndex={0}
...
export function DockHandle({ onOpen }: { onOpen(): void }) {
  const [wide, setWide] = useState(
    () => typeof window !== "undefined" && window.innerWidth >= DOCK_MIN_VIEWPORT,
  );
  useEffect(() => {
