...
import { loadCode, loadCompleted, loadRevealStage, resetExercise, saveCode, saveCompleted, saveRevealStage } from "../state/progress";
...
export function ExercisePage({ exercise }: { exercise: Exercise }) {
	...
	const resetToSkeleton = () => {
		if (!window.confirm("Replace your code with the original skeleton? Your edits, your revealed hints and this exercise's passed mark will be lost. Other modules are unaffected.")) return;
	...
	return (
		<section className="exercise">
			...
			<div className={fullscreen ? "workbench workbench-fullscreen" : "workbench"} aria-busy={running} ref={workbenchRef}>
				{/* The placeholder carries the editor's own height, so the page geometry
            is the same before and after it arrives. */}
				{editorNeeded ? (
					<Suspense fallback={<EditorPlaceholder />}>
						<CodeEditor initialDoc={initialDoc} onChange={(doc) => saveCode(exercise.id, doc)} handleRef={editorRef} onReady={setEditorReady} />
					</Suspense>
				) : (
					<EditorPlaceholder />
				)}
				...
				<div className="exercise-controls">
					...
					<button className="button-secondary workbench-expand" onClick={() => setFullscreen((f) => !f)}>
            ...
            underneath the page on every click. */}
				{ranOnce && (
					<div className="output-panel">
						...
						<pre ref={outputRef}>
							...
							{output.length ? output.join("\n") : running ? "(waiting for output...)" : "(your code printed nothing; add print(...) anywhere to inspect values)"}
						</pre>
						{scratchError && (
				...
				{result && <TestResults result={result} flagship={exercise.flagship} stale={stale} />}
			</div>
...
function TestResults({ result, flagship, stale }: { result: TestRunResult; flagship?: { test: string; note: string }; stale: boolean }) {
	...
	const nothingWritten = failures.length > 0 && passedCount === 0 && failures.every((g) => g.message.includes("NotImplementedError"));
	const flagshipPassed = flagship && result.tests.some((t) => t.name === flagship.test && t.passed);
	return (
		<div className={stale ? "test-results test-results-stale" : "test-results"}>
			{stale && <p className="test-stale-note">From an earlier run. Press Run tests to check the code as it stands now.</p>}
			<p className={result.passed ? "test-summary test-summary-pass" : "test-summary"}>{result.passed ? "All tests passed." : `${passedCount} of ${result.tests.length} tests passed.`}</p>
			{nothingWritten && (
				<p className="test-orient">
					Nothing is implemented yet, so every test stopped at the first function it called. Work down the list below: the earlier functions are the ones the later tests need.
				</p>
			)}
